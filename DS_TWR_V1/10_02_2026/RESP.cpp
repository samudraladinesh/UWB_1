...
static void final_msg_get_ts64(const uint8_t *ts_field, uint64_t *ts)
{
    *ts = 0;
    for (int i = 0; i < 8; i++) {
        *ts |= ((uint64_t)ts_field[i]) << (8 * i);
    }
}
   ...
   void loop () {
    while (1)
    {
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
            ...
            DBG("B1: Frame len = " + String(frame_len));
            ...
            if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
            {
              ...
              //resp_tx_time =(poll_rx_ts +(POLL_RX_TO_RESP_TX_DLY_UUS * UUS_TO_DWT_TIME)) >> 8;
                resp_tx_time = (poll_rx_ts + (1200 * UUS_TO_DWT_TIME)) >> 8;
                ...
                if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
                {
                    ...
                    DBG("E1: Final len = " + String(frame_len));
                    ...
                    if (memcmp(rx_buffer, rx_final_msg, ALL_MSG_COMMON_LEN) == 0)
                    {
                        ...
                        uint64_t poll_rx_ts_64, resp_tx_ts_64, final_rx_ts_64;
                        double Ra, Rb, Da, Db;
                        int64_t tof_dtu;
                        ...
                        final_msg_get_ts64(
                            &rx_buffer[FINAL_MSG_POLL_TX_TS_IDX], &poll_tx_ts);
                        final_msg_get_ts64(
                            &rx_buffer[FINAL_MSG_RESP_RX_TS_IDX], &resp_rx_ts);
                        final_msg_get_ts64(
                            &rx_buffer[FINAL_MSG_FINAL_TX_TS_IDX], &final_tx_ts);

                        DBG("F1: TS extracted");
                        DBG("poll_tx_ts = " + String((uint32_t)poll_tx_ts));
                        DBG("resp_rx_ts = " + String((uint32_t)resp_rx_ts));
                        DBG("final_tx_ts = " + String((uint32_t)final_tx_ts));

                        poll_rx_ts_64 = poll_rx_ts;
                        resp_tx_ts_64 = resp_tx_ts;
                        final_rx_ts_64 = final_rx_ts;

                        Ra = (double)(resp_rx_ts - poll_tx_ts);
                        Rb = (double)(final_rx_ts_64 - resp_tx_ts_64);
                        Da = (double)(final_tx_ts - resp_rx_ts);
                        Db = (double)(resp_tx_ts_64 - poll_rx_ts_64);
                        ...
                        DBG("F2: DIST = " + String(distance * 100));

                        Serial.println(
                            "0 " + String(distance * 100) + " 200");

                        Sleep(RNG_DELAY_MS - 10);
                    }
                    else
                    {
                        DBG("E2: FINAL FORMAT FAIL");
                    }
                }
                else
                {
                    DBG("D1: FINAL RX TIMEOUT/ERR");
                    dwt_write32bitreg(
                        SYS_STATUS_ID,

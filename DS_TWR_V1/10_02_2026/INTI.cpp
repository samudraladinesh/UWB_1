    ...
    void loop()
{
    ...
    while (1)
    {
        ...
        /* POLL sent, increment sequence number */
        frame_seq_nb++;

        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
            /* Clear RX good frame + TX done */
            dwt_write32bitreg(
                SYS_STATUS_ID,
            ...
            DBG("I3a: RESP len = " + String(frame_len));
            ...
            if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                DBG("I4a: poll_tx_ts = " + String((uint32_t)poll_tx_ts));
                DBG("I4b: resp_rx_ts = " + String((uint32_t)resp_rx_ts));
                ...
                /* Compute FINAL TX timestamp */
                final_tx_ts =
                    (((uint64_t)(final_tx_time & 0xFFFFFFFEUL)) << 8) +
                    TX_ANT_DLY;

                DBG("I5: FINAL TX scheduled");
                ...
                dwt_writetxfctrl(
                    sizeof(tx_final_msg) + FCS_LEN, 0, 1);
                ...
                if (ret == DWT_SUCCESS)
                {
                    ...
                    dwt_write32bitreg(
                        SYS_STATUS_ID, SYS_STATUS_TXFRS_BIT_MASK);

                    DBG("I6: FINAL TX DONE");

                    frame_seq_nb++;
                }
                else
                {
                    DBG("I5e: FINAL TX FAILED (late)");
                }
            }
            else
            {
                DBG("I4: RESP FORMAT FAIL");
            }
        }
        else
        {
            DBG("I2e: RESP RX TIMEOUT / ERR");
        ...
        /* Delay before next ranging exchange */
        Sleep(RNG_DELAY_MS);
    }
}
